...
% 230402: Changed from nFP = length(beh(x).FP);
nFP = length(beh(1).FP); % number of photometry signals
...
for x = 1:nAn
    ...
    norm = (tmp - bb)./(mm - bb);                                             % normalize
    norm = norm(find(time == winRew(1)):find(time == winRew(2)),:);           % extract reward window
    a_rew_norm{2}(:,x) = norm;                                                % normalized FP amplitude                                             % non-normalized FP amplitude
...
subplot(2,3,3); hold on
    ...
    xlabel('time to rew (s)'); xlim([-0.2 2]); xticks([0 0.5 1.0 1.5 2.0]);
