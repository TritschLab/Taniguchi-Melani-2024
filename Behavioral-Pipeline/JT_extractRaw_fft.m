function [rawS] = JT_extractRaw_fft(varargin)
...
%
    ...
    pickFP = menu('Select photometry signal','blank','unilateral-DA or bilateral-DA (NAc/Left)','blank2','bilateral-DA (DLS/Right)');
    ...
    if nargin ~= 1 % If need to load data into new structure
        ...
        for f = 1:length(fName)
            ...
            if isfield(data,'final')
                ...
                %catch % If no movement data exists or mouse is still
                %    rawS(x).on = []; rawS(x).off = []; rawS(x).onRest = []; rawS(x).offRest = [];
                %end
            end
        ...
        % 231019 UPDATE: If no onset/offset indicies despite having wheel data, this means that the mouse was resting the entire time.
        for y = 1:length(rawS)
            if (isempty(rawS(y).onRest) && isempty(rawS(y).offRest))
                rawS(y).onRest = 1;
                try
                    rawS(y).offRest = length(rawS(y).rawFP{1,1});
                catch
                    rawS(y).offRest = length(rawS(y).rawFP);
                end
            end
        end
    end
        ...
        case 4
            for x = 1:length(rawS)
                try
                    ...
                    rawS(x).fp_sub = rawS(x).rawFP; % full trace
                end
                try
                    rawS(x).fp_lbl = rawS(x).FPnames{pickFP}; % full traces
                catch
                    rawS(x).fp_lbl = rawS(x).FPnames; % full traces
                end
