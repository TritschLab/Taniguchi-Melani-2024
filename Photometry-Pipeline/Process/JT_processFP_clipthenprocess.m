...
for n = 1:nAcq
    ...
    for x = 1:nFP
        ...
        data.final(n).masked_FP{x}=[data.final(n).FP{x,1}(1:round(params.mask_win(1)/params.dsRate)-1); masking_vec; data.final(n).FP{x,1}(round(params.mask_win(1)/params.dsRate):end)];
        data.final(n).masked_nbFP{x}=[data.final(n).nbFP{x,1}(1:round(params.mask_win(1)/params.dsRate)-1); masking_vec; data.final(n).nbFP{x,1}(round(params.mask_win(1)/params.dsRate):end)];
        data.final(n).masked_FPbaseline{x}=[data.final(n).FPbaseline{x,1}(1:round(params.mask_win(1)/params.dsRate)-1); masking_vec; data.final(n).FPbaseline{x,1}(round(params.mask_win(1)/params.dsRate):end)];
    end
    timeVec = [1:L]/Fs;
