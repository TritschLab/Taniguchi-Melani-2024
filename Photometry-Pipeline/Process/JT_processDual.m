function [data] = JT_processDual(data,params)
...
for x = 1:nAcq
    ...
    for y = 1:nFP
        ...
        % 230613 UPDATE: (THIS DIDNT CHANGE OUTPUT)
        % Changed refSig to refSig(y) because I need to
        % selectively choose the appropriate ref Signal in order to do the
        % demodulation. Signal demoluation requires the exact frequency
        % provided by the raw ref Signal (i.e. 217 vs 217.001 is enough to
        % mess up the rest of the analysis due to remaining non-demodulated
        % data, and additionally the amplitude and offset. Hence, we need
        % to choose the correct refSig (in additional to specifying the
        % correct frequency in order to proceed with the analysis).
        ref = findRef(modFreq,refSig(y),rawFs); %Find the reference signal from the refsig array using modulation frequency
